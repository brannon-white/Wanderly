...
import { TRIP_VIBES } from '@/constants/tripVibes';
...
export default function TripVibesScreen() {
  ...
  const { tripVibes, setTripVibes } = useTripPlanning();

  function toggle(label: string) {
    if (tripVibes.includes(label)) {
      setTripVibes(tripVibes.filter(v => v !== label));
    } else {
      setTripVibes([...tripVibes, label]);
    }
  }
    ...
    <View style={shared.container}>
      <View style={[shared.topBar, { paddingTop: insets.top + 12 }]}>
        ...
        <View style={shared.progressBarTrack}>
          <View style={[shared.progressBarFill, { width: '55%' }]} />
        </View>
        <TouchableOpacity
          onPress={() => navigation.navigate('TripActivities')}
      ...
      >
        <Text style={shared.heading}>What's the vibe?</Text>
        <Text style={shared.subheading}>
          These shape the entire character of your itinerary. Pick everything that fits.
        </Text>

        <View style={styles.pillsGrid}>
          {TRIP_VIBES.map((item) => {
            const selected = tripVibes.includes(item.label);
            return (
              <Pill
                key={item.label}
                label={item.label}
